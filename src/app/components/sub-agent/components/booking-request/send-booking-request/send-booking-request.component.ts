...
@Component({
  selector: 'app-send-booking-request',
  templateUrl: './send-booking-request.component.html',
  styleUrls: ['./send-booking-request.component.scss']
})
export class SendBookingRequestComponent implements OnInit {
  ...
  constructor(private commonApiService:ApiServiceSubAgent,
  ...
  getShareLinks(){
    this.commonApiService.getSharedHistory().subscribe(data =>{
  ...
  dataset(data){
    data.forEach(element => {
      this.companies.push(element.shared_to_agency)
    });
  ...
  filterReservation(){
    ...
    if(this.company != ""){
      companyFilter = prodFilter.filter(x => x.shared_to_agency == this.company );
    }else{
